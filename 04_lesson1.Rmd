...
6. View `thesis.df` in the Environment panel
    ...
    + Click on the drop-down menu (the blue circle to the left of `thesis.df`). 
        ...
        + What are the first, second, and third `unique_id`'s represented in the dataframe and which `richness` values are associated with each `unique_id`?
    + Click on the button to view the data (the small grided table to the far-right of `thesis.df`)
        + Filter the dataframe to only see data from Onondaga Lake (`lake` = "onon")
