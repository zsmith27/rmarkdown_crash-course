...
The `fig.width` and `fig.height` options allow you to control the size of the figure generated from a given code chunk. `fig.cap` provides the ability to add a caption below the figure gnerated from a given code chunk.
...
__Which would render to:__ The mean sepal length found in the _iris_ dataset is `r mean(iris$Sepal.Length)`.
...
```
5. Add a code chunk below the "Preproccessing" header and add the following __DT__ code
...
6. Add a code chunk below the "Study Area" header and add the following __leaflet__ code
...
10. __Bonus:__ If you have enough time, edit the scatter plot code chunk to make the figure interactive with __plotly__
